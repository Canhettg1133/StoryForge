...
const LOCATION_PREFIXES = [
  'truong', 'thpt', 'thcs', 'dai hoc', 'hoc vien',
  'thon', 'lang', 'xa', 'huyen', 'quan',
  'thanh pho', 'tp', 'pho', 'duong', 'ngo',
  'benh vien', 'nha tho', 'nghia trang',
  'nha may', 'khu', 'toa nha', 'biet thu',
  'khach san', 'chung cu', 'ham mo',
...
function shouldTreatAsLocation(name) {
  ...
  return LOCATION_PREFIXES.some((prefix) => normalized.startsWith(prefix));
}
...
function extractLocationCandidatesFromSentence(sentence) {
  ...
  for (const match of source.matchAll(prepositionPattern)) {
    ...
    const capped = value.split(' ').slice(0, 8).join(' ');
...
export function enrichWithIncidentIntelligence(result = {}, chunks = [], options = {}) {
  ...
  const linked = linkEventLocations(result, chunks, extracted.locations, mergedOptions);
