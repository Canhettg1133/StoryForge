...
function paddedByte(bytes, index, totalLength, bitLengthHigh, bitLengthLow) {
  if (index < bytes.length) return bytes[index];
  if (index === bytes.length) return 0x80;
  const lengthOffset = index - (totalLength - 8);
...
function sha256HexFallback(input) {
  ...
  for (let offset = 0; offset < totalLength; offset += 64) {
    for (let index = 0; index < 16; index += 1) {
      ...
      words[index] = (
        (paddedByte(bytes, byteOffset, totalLength, bitLengthHigh, bitLengthLow) << 24)
        | (paddedByte(bytes, byteOffset + 1, totalLength, bitLengthHigh, bitLengthLow) << 16)
        | (paddedByte(bytes, byteOffset + 2, totalLength, bitLengthHigh, bitLengthLow) << 8)
        | paddedByte(bytes, byteOffset + 3, totalLength, bitLengthHigh, bitLengthLow)
      ) >>> 0;
    }
