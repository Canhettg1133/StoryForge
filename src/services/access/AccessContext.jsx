...
export function AccessProvider({ children }) {
  ...
  const refreshAccess = useCallback(async ({ token, session, silent = false } = {}) => {
    ...
    try {
      ...
      const fallback = token || session?.user
        ? setCachedAccessSnapshot(createAuthenticatedAccessFallbackSnapshot(session), token || session?.access_token || '')
        : setCachedAccessSnapshot(createUnauthenticatedAccessSnapshot(), '');
      setAccess(fallback);
